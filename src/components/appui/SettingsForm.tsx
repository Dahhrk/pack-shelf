  ...
  return (
    <div className="relative">
      <div className="rounded-xl border border-stone-800 bg-stone-900 divide-y divide-stone-800">
        {sections.map((section) => (
          <div key={section.title} className="p-6">
            <h4 className="text-sm font-mono text-stone-500 uppercase tracking-widest mb-4">
              {section.title}
            </h4>
                ...
                return (
                  <div key={field.key} className="flex items-center gap-4">
                    <label
                      ...
                      className={`text-sm w-40 shrink-0 ${dirty ? "text-amber-400" : "text-stone-400"}`}
                    >
                      {field.label}
                    ...
                    {field.type === "text" && (
                      <input
                        ...
                        className="focus-ring flex-1 bg-stone-800 border border-stone-700 rounded-lg px-3 py-2 text-sm text-stone-200 outline-none focus:border-amber-500 transition-colors"
                      />
                    )}
                    {field.type === "select" && (
                      <select
                    ...
                    {field.type === "toggle" && (
                      <button
                        ...
                        className={`focus-ring relative w-10 h-6 rounded-full transition-colors ${
                          values[field.key] ? "bg-amber-500" : "bg-stone-700"
                        }`}
                      >
                        <motion.div
      ...
      <AnimatePresence>
        {(isDirty || saved) && (
          <motion.div
            className="mt-4 flex items-center justify-between rounded-xl border border-stone-700 bg-stone-800 px-4 py-3"
            ...
            {saved ? (
              ...
              <>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={handleUndo}
                    className="focus-ring flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-stone-600 text-sm text-stone-300 hover:bg-stone-700 transition-colors"
                  >
                    <Undo2 className="w-3.5 h-3.5" /> Undo
                  </button>
                  <button
                    onClick={handleSave}
                    className="focus-ring flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-600 text-sm text-white hover:bg-amber-500 transition-colors"
                  >
